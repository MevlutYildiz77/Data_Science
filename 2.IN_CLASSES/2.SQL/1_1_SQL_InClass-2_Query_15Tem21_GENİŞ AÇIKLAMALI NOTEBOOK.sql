...
-- þimdi product_id'si prodcut.stock tablosunda olmayan ürünleri seçerek bu iþlemi yapalým.
SELECT B.store_id, A.product_id, A.product_name, 0 quantity -- quantity adýnda sütun oluþturup içine 0 giriyor. çünkü stokta olmayan ürünlerin id leri ile cross join yapacaðýz.
FROM product.product AS A
CROSS JOIN sales.stores AS B
WHERE A.product_id NOT IN (SELECT product_id FROM product.stock) -- product_id'si stoklarda olmayan ürünlerin product_id'lerini seç.
